"""Tests for MetaMedQA Verifiers evaluator.

Tests cover dataset loading, environment construction, parser configuration,
rubric wiring, and text-based answer handling.
"""

from __future__ import annotations

from typing import TYPE_CHECKING
from unittest.mock import MagicMock, patch

from med_reason_evals.verifiers.metamedqa import MetaMedQAEvaluator
from med_reason_evals.verifiers.utils.prompts import AnswerFormat


if TYPE_CHECKING:
    from collections.abc import Callable

    from datasets import Dataset


class TestMetaMedQADatasetLoading:
    """Tests for MetaMedQA dataset loading."""

    def test_load_datasets_no_train_split(
        self,
        mock_load_dataset_factory: Callable[[Dataset], MagicMock],
        metamedqa_mock_dataset: Dataset,
    ) -> None:
        """Test that MetaMedQA has no train split."""
        with patch(
            "med_reason_evals.data.metamedqa.load_dataset",
            mock_load_dataset_factory(metamedqa_mock_dataset),
        ):
            evaluator = MetaMedQAEvaluator(streaming=False)
            train_ds, eval_ds = evaluator._load_datasets()

        assert train_ds is None
        assert eval_ds is not None

    def test_text_answer_mapping(
        self,
        mock_load_dataset_factory: Callable[[Dataset], MagicMock],
        metamedqa_mock_dataset: Dataset,
    ) -> None:
        """Test that text-based answers are mapped correctly."""
        with patch(
            "med_reason_evals.data.metamedqa.load_dataset",
            mock_load_dataset_factory(metamedqa_mock_dataset),
        ):
            evaluator = MetaMedQAEvaluator(streaming=False)
            _, eval_ds = evaluator._load_datasets()

            # Check answers are text-based (not letters)
            assert "answer" in eval_ds.column_names


class TestMetaMedQAEnvironment:
    """Tests for MetaMedQA environment construction."""

    def test_environment_construction(
        self,
        mock_load_dataset_factory: Callable[[Dataset], MagicMock],
        metamedqa_mock_dataset: Dataset,
        assert_env_has_basic_fields: Callable,
    ) -> None:
        """Test environment is constructed correctly."""
        with patch(
            "med_reason_evals.data.metamedqa.load_dataset",
            mock_load_dataset_factory(metamedqa_mock_dataset),
        ):
            evaluator = MetaMedQAEvaluator(streaming=False)
            env = evaluator.environment()

            assert_env_has_basic_fields(env)


class TestMetaMedQAParserConfiguration:
    """Tests for MetaMedQA parser configuration."""

    def test_default_xml_format(
        self,
        mock_load_dataset_factory: Callable[[Dataset], MagicMock],
        metamedqa_mock_dataset: Dataset,
        assert_parser_is_xml: Callable,
    ) -> None:
        """Verify default format is XML."""
        with patch(
            "med_reason_evals.data.metamedqa.load_dataset",
            mock_load_dataset_factory(metamedqa_mock_dataset),
        ):
            evaluator = MetaMedQAEvaluator(streaming=False)
            env = evaluator.environment()

            assert evaluator.answer_format == AnswerFormat.XML
            assert_parser_is_xml(env.parser, has_think=False)


class TestMetaMedQARubricConfiguration:
    """Tests for MetaMedQA rubric configuration."""

    def test_rubric_has_accuracy_reward(
        self,
        mock_load_dataset_factory: Callable[[Dataset], MagicMock],
        metamedqa_mock_dataset: Dataset,
        assert_rubric_has_one_func_weight_one: Callable,
    ) -> None:
        """Verify rubric uses accuracy_reward."""
        with patch(
            "med_reason_evals.data.metamedqa.load_dataset",
            mock_load_dataset_factory(metamedqa_mock_dataset),
        ):
            evaluator = MetaMedQAEvaluator(streaming=False)
            env = evaluator.environment()

            assert_rubric_has_one_func_weight_one(
                env.rubric, func_name="accuracy_reward"
            )
