"""Tests for MetaMedQA Verl evaluator.
...
from med_reason_evals.verl import MetaMedQAEvaluator
...
class TestMetaMedQAMain:
    """Tests for MetaMedQA main() function."""
    ...
    async def test_main_prints_results(self, mocker):
        ...
        mock_evaluator_class = mocker.patch(
            "med_reason_evals.verl.metamedqa.MetaMedQAEvaluator"
        )
        ...
        from med_reason_evals.verl.metamedqa import main
        ...
        assert "MetaMedQA Verl Results:" in calls[0][0][0]
        assert "0.550" in str(calls[4])
...
class TestMetaMedQALoadDataset:
    """Tests for MetaMedQA _load_dataset method."""

    def test_load_dataset(self, mocker, metamedqa_verl_dataset: Dataset):
        """Test _load_dataset method."""
        mock_dataset_class = mocker.patch(
            "med_reason_evals.verl.metamedqa.MetaMedQADataset"
        )
        mock_dataset_instance = MagicMock()
        mock_verl_dataset = metamedqa_verl_dataset
        ...
        with patch.dict("os.environ", {"GROQ_API_KEY": "test-key"}):
            evaluator = MetaMedQAEvaluator(split="train")
    ...
    def test_load_dataset_has_required_fields(
        self, mocker, metamedqa_verl_dataset: Dataset
    ):
    ...
    def test_load_dataset_non_streaming(self, mocker, metamedqa_verl_dataset: Dataset):
...
class TestMetaMedQABuildResult:
    """Tests for MetaMedQA _build_result method."""

    def test_build_result(self, assert_verl_result_shape: Callable):
        """Test _build_result method."""
        with patch.dict("os.environ", {"GROQ_API_KEY": "test-key"}):
            evaluator = MetaMedQAEvaluator(split="test")
            avg_score = 0.55
